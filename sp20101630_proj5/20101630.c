...
void command_tokenizing (void) {
	...
	strcpy(preserve,command);

	token = strtok(preserve,separator);
	while (token != NULL) {
		strcpy(parameter[parameter_count++],token);
...
void command_analyser (void) {
	...
	case 16: progaddress(); break;		//progaddr 명령어
	case 17: linking_loader(); break;	//loader 명령어
	case 18: run(); break;			//run 명령어
	case 19: bp(); break;			//bp 명령어
	}
...
void progaddress (void) {
	...
	// Assign 하는 값이 16진수가 아니라면 에러
	if (isHex(parameter[1],&progaddr) == 0) {
		printf("ERROR: Input Address is not hexadecimal.\n");
		return;
	}

	if (progaddr < 0 || progaddr >= MAX_MEMORY) {
		printf("ERROR: Out of Memory!\n");
		return;
	}

	// 설정이 되었으면 출력
	printf("\tPROGADDR is assigned as 0x%06X\n",progaddr);
	Lndex = 0;
...
// 리 턴 값: 없음
// 파라미터: 없음
// 목    표: 입력 된 obj파일들을 읽어들여, 링킹작업을
//   	     하며, 그 후 PROGADDR 위치로 로드합니다.
//-----------------------------------------------//
void linking_loader (void) {
	...
	// 변수 설정
	int initial_address, csaddr;
	int i, j, program_length, count, check, m;
	unsigned int k;
	char inputLine[MAX], temp[MAX], program_name[MAX], optr;

	FILE *opener = NULL;
	hash_node *hash_cursor, *current, *previous;
	...
	// 입력 된 파일 순서대로 읽어들여 ESTAB을 완성하는 For문
	for (i = 1; i < parameter_count; i++) {
		// 파일을 읽어들임
		opener = fopen(parameter[i],"r");
		if (opener == NULL) {
			printf("ERROR: There isn't file [%s]\n",parameter[i]);
			isError = -1;
			continue;
		}	

		// 한 줄씩 읽어들이면서 H,E,D 레코드를 탐색
		count = 0;
		while (fgets(inputLine,MAX,opener) != NULL) {
			// 만약 줄바꿈 관련 특수문자를 모두 제거 
			...
			// H와 E 레코드가 나오면 count를 늘려주며, 항상 count는 2로 끝나야함.
			if (inputLine[0] == 'H') { count++; }
			if (inputLine[0] == 'E') { count++; }

			// H 레코드일 경우, 프로그램의 이름과 길이를 알아내고,
			// 프로그램의 이름을 ESTAB에 저장
			if (inputLine[0] == 'H') {
				k = 0;
				for (j = 1; j < 7; j++) {
					if (inputLine[j] != ' ') {
						program_name[k++] = inputLine[j];
					}
				}program_name[k] = '\0';

				// ESTAB에 프로그램 이름 저장
				hash_cursor = hash_add(&estab,program_name,csaddr,-1);
				strcpy(hash_cursor->value,program_name);
				
				// 프로그램의 길이를 program_length에 저장
				sscanf(inputLine + 13,"%x",&program_length);
				hash_cursor->format = program_length;
			}
			// D 레코드일 경우
			else if (inputLine[0] == 'D') {
				for (j = 1; j < strlen(inputLine); j+=12) {
					if (inputLine[j] == '\n') break;
					if (inputLine[j] == '\r') break;

					// Define 된 Symbol과 그 Address를 같이 ESTAB에 저장
					strncpy(temp,inputLine + j,6); temp[6] = '\0';
					for (k = 0; k < strlen(temp); k++) {
						if (temp[k] == ' ') { temp[k] = '\0'; break; }
					}

					// 만약 ESTAB에 이미 존재한다면, Duplicate Symbol 에러
					hash_cursor = hash_pointer(&estab,temp,0);
					if (hash_cursor != NULL) {
						printf("ERROR: Duplicate External Symbol [%s] !\n",temp);
						isError = -1;
					} else {
						hash_cursor = hash_add(&estab,temp,csaddr,-1);
					
						// 새로 추가 된 SYMBOL에 값 저장
						strncpy(temp,inputLine + j + 6,6); temp[6] = '\0';
						sscanf(temp,"%x",&k);
						hash_cursor->opcode += k;
						strcpy(hash_cursor->value,program_name);
					}
				}
			}
			// E 레코드일 경우
				...
				// CSECT로 들어온 obj일 경우, 맨 뒤 Address가 없을 수 있다
				if (strlen(inputLine) > 1) {
					strcpy(temp,inputLine+1);
					sscanf(temp,"%x",&runaddr);
				}

				// CSADDR에 program_length를 더해주어, 다음 CSADDR을 얻어냄
				csaddr += program_length;
				program_length = 0;
				program_name[0] = '\0';
			}
		}
		// H레코드와 E레코드가 둘 중 하나라도 없으면 에러
		if (count != 2) {
			printf("ERROR: Wrong Object File [%s], Can't Loading.\n",parameter[i]);
			isError = -1;
		}
		fclose(opener);
	}

	// 프로그램을 올리려고 보니, 메모리를 벗어날경우
	if (csaddr > MAX_MEMORY) {
		printf("ERROR: Loader: Out of Memory!\n");
		isError = -1;
		return;
	}

	// Pass2
	initial_address = progaddr;
	for (i = 1; i < parameter_count; i++) {
		// 파일을 순서대로 읽어들임
		opener = fopen(parameter[i],"r");
		if (opener == NULL) {
		...
		// count는 Reference number가 있는지 없는지 구분되는 변수
		count = 6;	
		while (fgets(inputLine,MAX,opener) != NULL) {
			// 줄바꿈 관련 특수문자를 모두 제거
			for (j = 0; j < strlen(inputLine); j++) {
			...
			// H레코드일 경우,
			if (inputLine[0] == 'H') {
				strncpy(temp,inputLine + 1, 6); temp[6] = '\0';
				for (j = 0; j < strlen(temp); j++) {
					if (temp[j] == ' ') { temp[j] = '\0'; break; }
				}

				// 프로그램의 이름이 ESTAB에 없다면 에러
				strcpy(program_name,temp);
				hash_cursor = hash_pointer(&estab,temp,0);
				if (hash_cursor == NULL) {
					printf("ERROR: File [%s]: Undifned Symbol [%s] is used.\n",parameter[i],temp);
					isError = -1;
				} else {
					// 로드가 되는 위치를 CSADDR과 initial_address에 저장
					csaddr		= hash_cursor->opcode;
					initial_address = hash_cursor->opcode;
				}
			}
			// R 레코드일 경우,
			else if (inputLine[0] == 'R') {
				// Reference Number가 있다면, count는 8로 변경하여
				// 이 obj파일은 Reference Number가 있는 파일임을 명시
				...
				for (j = 1; j < strlen(inputLine); j+=count) {
					if (inputLine[j] == '\n') break;
					if (inputLine[j] == '\r') break;

					// count에 따라 SYMBOL의 위치가 다르므로 따로 구분
					if (count == 8) {
						// reference Number가 있을 경우, check 변수에 저장
						strncpy(temp,inputLine + j,2); temp[2] = '\0';
						sscanf(temp,"%d",&check);

						strncpy(temp,inputLine + j + 2,6); temp[6] = '\0';
					} else {
						strncpy(temp,inputLine + j,6); temp[6] = '\0';
					}
					for (k = 0; k < strlen(temp); k++) {
					...
					// ESTAB에서 해당 Symbol을 찾아보고
					hash_cursor = hash_pointer(&estab,temp,0);
					// 없다면 Undefined Error
					if (hash_cursor == NULL) {
						printf("ERROR: File [%s]: External Reference Undefined Symbol [%s] is used.\n",parameter[i],temp);
						isError = -1;
					// 있다면, count가 8일 경우에만, check 값을 ESTAB에 저장
					} else {
						if (count == 8) {
							hash_cursor->format = check;
						}
					}
				}
			}
			// T레코드일 경우, 일반적인 로드를 진행
			else if (inputLine[0] == 'T') {
				// 오류가 있다면 건너뜀.
				if (isError == -1) continue;
				// T레코드의 로딩이 시작 될 Address를 읽어오고
				strncpy(temp,inputLine + 1, 6); temp[6] = '\0';
				sscanf(temp,"%x",&k);

				// 로드를 2글자씩 진행
				initial_address = k + csaddr;
				if (execaddr < 0) execaddr = initial_address;
				for (j = 9; j < strlen(inputLine); j+=2) {
					if (inputLine[j] == '\n') break;
					if (inputLine[j] == '\r') break;

					// 2글자를 읽어옴
					temp[0] = inputLine[j];
					temp[1] = inputLine[j+1];
					temp[2] = '\0';

					// 혹시나 16진수가 아니라면 에러
					if (!isHex(temp,&check)) {
						printf("ERROR: File [%s]: Object-Code [%s] is not Hexadecimal.\n",parameter[i],temp);
						isError = -1;
						count = 0x00;
					}

					// 메모리에 로딩
					memory[initial_address++] = check;
				}
			}
			// M레코드일 경우, 값 조정을 진행
				...
				// 수정해야할 주소의 위치를 얻음
				strncpy(temp,inputLine + 1,6); temp[6] = '\0';
				sscanf(temp,"%x",&k); k += csaddr;

				// 05 혹은 06 의 수정해야할 Hex 갯수를 얻음
				strncpy(temp,inputLine + 7,2); temp[2] = '\0';
				sscanf(temp,"%x",&j);

				// 만약 M레코드가 짧다면, 이는 일반적인 obj파일
				if (strlen(inputLine) <= 10) {
					optr = '+';
					strcpy(temp,program_name);
				// 아니라면 Reference Number 혹은 +LABEL의 obj파일
				} else {
					optr = inputLine[9];
					strcpy(temp,inputLine + 10);
				}

				// count가 8이라면, reference Number가 있는 것
				if (count == 8) {
					// check에 reference Number를 받아옴
					sscanf(temp,"%d",&check);

					// check가 1 이라면, 자기자신의 program_name을 더해야함
					if (check == 1) {
						strcpy(temp,program_name);
						hash_cursor = hash_pointer(&estab,temp,0);
					} else {
					// 그런게 아니라면 해당 reference number를 가지는 label 탐색
						hash_cursor = estab;
						while (hash_cursor != NULL) {
							if (hash_cursor->format == check) break;
							else hash_cursor = hash_cursor->nextNode;
						}
					}
				// count가 6이라면, program_name을 가져옴
				} else {
					hash_cursor = hash_pointer(&estab,temp,0);
				}
				// 만약 해당 라벨 혹은 reference number가 ESTAB에 없다면 Undefined Error
				if (hash_cursor == NULL) {
					printf("ERROR: File [%s]: Undefined Symbol [%s] in Modification Record.\n",parameter[i],temp);
					isError = -1;
				} else {
					// Modification이 05 혹은 06일때 나눠서 계산
					m = 0;

					// 05 일 경우, 메모리에서 5자리를 가져옴
					if (j == 5) {
						m += (memory[k]%0x10) * 0x10000;
						m += memory[k+1] * 0x100;
						m += memory[k+2];
						if (m >= 0x80000) {
							m -= 1;
					...
					// operator가 + 혹은 - 일 경우, 나눠서 계산
					if (optr == '+') {
						m += hash_cursor->opcode;

						// 연산 값이 음수라면 2's complement
						if (m < 0) {
							if (j == 5) {
								m *= -1;
								m ^= 0xFFFFF;
						...
						// 연산 된 값을 메모리에서 갱신
						if (j == 5) {
							memory[k] = ((memory[k] / 0x10) * 0x10) + (m / 0x10000);
						} else {
							memory[k] = (m / 0x10000);
						}
						// 메모리에 갱신 된 값이 저장 됨
						memory[k+1] = (m / 0x100) % 0x100;
						memory[k+2] = (m % 0x100);
					} else if (optr == '-') {
						m -= hash_cursor->opcode;
	...
	// Load Map을 출력하는 부분, isError가 0. 즉, 에러가 없었어야 한다.
	if (isError == 0) {
		// 카테고리를 출력하고
		...
		// ESTAB을 탐색하면서 순서대로 출력
		temp[0] = '\0'; k = 0;
		current = estab;
		while (current != NULL) {
			if (strcmp(temp,current->value) == 0) {
				printf("\t\t");
			} else {
				strcpy(temp,current->value);
				printf("%s\t\t",current->value);
			}
			if (strcmp(current->value,current->mnemonic) == 0) {
				printf("\t\t");
			} else {
				printf("%s\t\t",current->mnemonic);
			}
			printf("%04X\t",current->opcode);	
			if (strcmp(current->value,current->mnemonic) != 0) {
				printf("\n");
			} else {
				printf("%04X\n",current->format);
...
// 리 턴 값: 없음
// 파라미터: 없음
// 목    표: 가상메모리에 올라간 프로그램을 실제로
//	     run시켜서 레지스터와 메모리 값의 변화를 확인
//-----------------------------------------------//
void run (void) {

	// 변수 초기화
	int l, r;
	static unsigned int objectcode, objecthead, objectcount, breakaddress, count;
       	unsigned int temp;
	char temp_str[MAX];

	static hash_node *hash_cursor;

	// Break가 아닌, 아예 새롭게 시작할경우
	// Lndex가 0 이면, 새로 run을 하는 경우로 인식
	if (Lndex == 0) {
		// 변수 초기화 및 L Register에 0xFFFFFF 입력
		register_array[reg_L] = 0xFFFFFF;
		register_L[Lndex++] = 0xFFFFFF;

		// 프로그램이 시작 될 주소는 PROGADDR
		register_array[reg_PC] = runaddr = progaddr;
		breakaddress = 0xFFFFFF;
		objectcode = objecthead = objectcount = count = 0;
		hash_cursor = NULL;
	}
	while (1) {
		// Program Counter의 값이 음수가 되면 종료
		if (runaddr >= 0xF00000) {
			// 레지스터 정보를 모두 출력하고 End Program.
			print_register();
			printf("\tEnd Program.\n");
			return;
		}
		...
		while (1) {
			// Invalid한 Opcode일 경우, 런타임에러 발생
			if (objectcount == count && objectcode != 0x00) break;
			if (objectcount >  count) {
				printf("\tERROR: Runtime: Invalid Objectcode.\n");
				isError = -1;
				Lndex = 0;
				return;
			}
			// break point를 만났다면, 그 자리에서 멈춤.
			if (breakp[runaddr] == 1 && breakaddress != runaddr) {
				print_register();
				printf("\tStop Running at [%04X]\n",runaddr);
				breakaddress = runaddr;
				return;
			}
			if (breakp[runaddr] == 1 && breakaddress == runaddr) {
				breakaddress = 0xFFFFFF;
			...
			// 처음으로 objectcode를 받을 때, opcode에 해당하는
			// 부분을 objecthead에 넣어주어 포맷을 알아낸다.
			objectcode *= 0x100;
			temp = memory[runaddr++];
			objectcode += temp;
			if (objectcount == 0) { 
				sprintf(temp_str,"%x%x",((temp/0x10)%0x10),(((temp%0x10)/8)*8)+((((temp%0x10)%8)/4)*4));
				sscanf(temp_str,"%x",&temp);

				objecthead = temp;
				count = 0;
			} else if (count == 3 && objectcount == 1) {
				// 포맷 3에서 e 비트가 1일 경우, 포맷 4로 변경
				if ((temp/0x10) % 2 == 1) {
					count = 4;
				}
			}
			if (hash_cursor == NULL) {
				// Mnemonic List를 통해서 어떤 Opcode 인지를 확인
				hash_cursor = hash_pointer(&mnemonic_list[hash_function(OpToMne,NULL,objecthead)],NULL,objecthead);
				if (hash_cursor == NULL) {
					printf("ERROR: ADDRESS [%4X]: Invalid Opcode.\n",runaddr - 1);
				} else {
					count = hash_cursor->format;
				}
			}
			objectcount++;
		}

		register_array[reg_PC] = runaddr;

		// ObjectCode를 Opcode에 따라 실행
		// 각각의 Case는 Opcode의 16진수 값을 나타냄
		switch (hash_cursor->opcode) {
		// assign , load, getaddress 의 함수로 구성되어 있으며,
		// 모든 Opcode는 이러한 3개의 함수의 응용으로 구성되어 있음.
		case 0x14: assign(getaddress(objectcode),register_array[reg_L],3); break;
		case 0x68: register_array[reg_B] = load(getaddress(objectcode),3,objectcode); break;
		case 0x48: register_array[reg_PC] = getaddress(objectcode);
			   register_array[reg_L] = runaddr;
			   register_L[Lndex++] = runaddr;
			   runaddr = register_array[reg_PC];
			   break;
		case 0xB4: register_array[((objectcode % 0x100)/0x10)] = 0; break;
		case 0x74: register_array[reg_T] = load(getaddress(objectcode),3,objectcode); break;
		case 0xE0: register_array[reg_SW] = -1; break;
		case 0x30: if (register_array[reg_SW] == 0) {
				   register_array[reg_PC] = getaddress(objectcode);
				   runaddr = register_array[reg_PC];
			   }
			   break;
		// I/O 관련 Insturction은 언제나 True가 되도록 설정.
		case 0xD8: register_array[reg_A] = 0; break;
		case 0xA0: l = ((objectcode % 0x100)/0x10);
			   r = ((objectcode % 0x100)%0x10);
			   register_array[reg_SW] = cmp(register_array[l],register_array[r]);
			   break;
		case 0x54: assign(getaddress(objectcode),register_array[reg_A],1); break;
		case 0xB8: l = (objectcode / 0x10) % 0x10;
			   if (register_array[reg_X] >= 0x800000)
				   register_array[reg_X] = -1*(0x1000000 - register_array[reg_X]); 
			   register_array[reg_X]++;
			   if (register_array[reg_X] < 0)
				   register_array[reg_X] = (0x1000000 - (-1*register_array[reg_X]));
			   register_array[reg_SW] = cmp(register_array[reg_X],register_array[l]);
			   break;
		case 0x38: if (register_array[reg_SW] == -1) {
				   register_array[reg_PC] = getaddress(objectcode);
				   runaddr = register_array[reg_PC];
			   }
			   break;
		case 0x10: assign(getaddress(objectcode),register_array[reg_X],3); break;
		case 0x4C: register_array[reg_PC] = register_L[--Lndex];  
			   runaddr = register_array[reg_PC];
			   break;
		case 0x00: register_array[reg_A] = load(getaddress(objectcode),3,objectcode); break;
		case 0x28: register_array[reg_SW] = cmp(register_array[reg_A],getaddress(objectcode)); break;
		case 0x50: register_array[reg_A] = load(getaddress(objectcode),1,objectcode); break;
		case 0xDC: break;
		case 0x3C: register_array[reg_PC] = getaddress(objectcode);
			   runaddr = register_array[reg_PC];
			   break;
		case 0x0C: assign(getaddress(objectcode),register_array[reg_A],3); break;
		case 0x04: register_array[reg_X] = load(getaddress(objectcode),3,objectcode); break;
		}
	
		// PC Register에 runaddr 값을 저장
		register_array[reg_PC] = runaddr;
...
// 함수이름: cmp
// 리 턴 값: 비교값을 -1,0,1
// 파라미터: 비교 할 2개의 값. l과 r
// 목    표: 값을 비교할 경우, 두 값을 비교하여 리턴
//-----------------------------------------------//
int cmp (int l, int r) {
	// l을 기준으로, 더 크면 1, 같으면 0
	// 작으면 -1이 됨.
	if (l >= 0x800000) {
		l = (0x1000000 - l) * -1;
	}
...
// 리 턴 값: 없음
// 파라미터: 없음
// 목    표: Breakpoint값을 설정하거나 비우는 역할 수행
//-----------------------------------------------//
void bp (void) {
	...
	// "bp"만 입력 되어 있을 경우, 현재의 break point를 출력
	if (parameter_count == 1) {
		printf("\tBreak Points\t\n");
		printf("----------------------------\n");
		...
		// "bp clear"가 입력 되었을 경우
		if (strcmp(parameter[1],"clear") == 0) {
			// bptable을 모두 비움
			current = bptable;
			while (current != NULL) {
				previous = current;
			...
			// break를 나타내는 breakp도 모두 초기화
			for (i = 0; i < MAX_MEMORY; i++) {
				breakp[i] = 0x00;
			}

			// 모든 bp가 사라졌다는 문구 출력
			printf("\t[SUCCESS] All Breakpoints are removed.\n");
		} else {
		// "bp HEX"가 입력 되었을 경우,
			// 16진수가 아니라면 에러
			if (!isHex(parameter[1],&ta)) {
				ERROR(ERR_HEX);
				return;
			} else {
				if (ta >= MAX_MEMORY) {
				...
				current = hash_pointer(&bptable,NULL,ta);
				if (current != NULL) {
					printf("ERROR: Duplicate Break Point [%06X].\n",ta);
					isError = -1;
					return;
				}
				hash_add(&bptable,"break",ta,0);

				// 문구를 출력
				printf("\t[SUCCESS] Create Breakpoint at 0x%04X\n",ta);
			}
		}
	}
}
...
void print_register (void) {
	...
	// reigster_array의 값에서 각각에 맞는 값을 출력
	printf("\tA: %06X\tX :%06X\n",register_array[reg_A],register_array[reg_X]);
	printf("\tL: %06X\tPC:%06X\n",register_array[reg_L],register_array[reg_PC]);
	printf("\tB: %06X\tS :%06X\n",register_array[reg_B],register_array[reg_S]);
	printf("\tT: %06X\n",register_array[reg_T]);

}
//-----------------------------------------------//
// 함수이름: getaddress
// 리 턴 값: unsigned int
// 파라미터: object-code를 나타내는 int값
// 목    표: object-code 값을 분석하여 Target
//	     Address를 구하여 리턴 시킴
//-----------------------------------------------//
unsigned int getaddress (unsigned int objectcode) {
	
	// 변수 초기화
	unsigned char addressing_method, reference_method;
	int result, temp, i;

	// 만약 Extened Format일 경우,
	if (objectcode > 0x1000000) {
		// Addressing은 Extended Mode이며,
		// Regrence를 구함.
		addressing_method = 0x1;
		reference_method = ((objectcode / 0x1000000) % 0x10) & 0x3;

		// 5개의 Hex를 Result로 구함
		result = objectcode % 0x100000;

		// 만약 indirect reference 라면, 그 값을 다시 참조
		if (reference_method == 2) {
			temp = 0;
			for (i = result; i < result + 3; i++) {
				temp *= 0x100;
				temp += memory[i];
			}
			result = temp;
	...
	// 일반적인 경우
		...
		// addressing 과 reference mode를 뽑아냄.
		addressing_method = (objectcode / 0x1000) % 0x10;
		reference_method = ((objectcode / 0x10000) % 0x10) & 0x3;

		// 각각의 addressing mode에 대하여, X , B , PC 값을 얻어냄
		if (addressing_method & 0x8) result += register_array[reg_X];	
		if (addressing_method & 0x4) result += register_array[reg_B];
		if (addressing_method & 0x2) result += register_array[reg_PC];

		// Operand에 있는 3개의 Hex 값을 읽어들임
		temp = objectcode % 0x1000;
		if (temp & 0x800) {
			temp -= 1;
			temp ^= 0xFFF;
			temp *= -1;
		...
		// indirect addressing일 경우, 메모리에서 그 값을 다시 참조
		if (reference_method == 2) {
			temp = 0;
			for (i = result; i < result + 3; i++) {
				temp *= 0x100;
...
//-----------------------------------------------//
// 함수이름: assign
// 리 턴 값: 성공적으로 끝났을 경우 1, 아닌 경우 -1
// 파라미터: Address와 입력할 Value, 입력 될 길이 Size
// 목    표: Memory에 값을 저장하는 역할을 수행
//-----------------------------------------------//
int assign (int address, int value, int size) {
	...
	// value의 값을 int에서 문자열로 전환
	temp[0] = '\0'; tmp[0] = '\0';
	// 음수 일경우, 2's complement 로 변환
	if (value < 0) {
		value *= -1;
		// size 크기만큼 2개씩 잘라서 문자열로 변환
		for (i = 0; i < size; i++) {
			value ^= 0xFF;
			sprintf(tmp,"%02X",value%0x100);
			strcat(tmp,temp);
			strcpy(temp,tmp);
			value /= 0x100;
		}
		// 2's complement로 변환 끝
		sscanf(temp,"%x",&value);
		value += 1;
	}
	...
	for (i = address; i < address + size; i++) {
		// tmp 값을 2글자씩 뽑아내어,
		strncpy(tmp,temp + ((i - address) * 2),2); tmp[2] = '\0';
		sscanf(tmp,"%x",&assign_value);
		// memory 에 저장시킴
		memory[i] = assign_value;
	}
...
//-----------------------------------------------//
// 함수이름: load
// 리 턴 값: 메모리에서 읽어들인 값을 리턴
// 파라미터: Address와 읽어들일 길이 size, objectcode
// 목    표: Memory에서 size 길이만큼 읽어들여 리턴하는 함수
//-----------------------------------------------//
int load (int address, int size, unsigned int objectcode) {

	// 변수 선언 및 초기화
	int i, temp = 0, reference_method = 0;

	if (address >= 0x800000 || address > MAX_MEMORY) {
		printf("ERROR: Outbound of Memory is Referenced.\n");	
		return 0;
	}
	...
	// reference mode가 simple 이거나 indirect이면 memory 에서 reference.
	if (reference_method >= 2) {
		for (i = address; i < address+size; i++) {
			temp *= 0x100;
			temp += memory[i];
		}
	// 아니라면 immediate이므로, 그 값을 그대로 리턴
	} else {
		temp = address;
	}
